...
The Taekwondo tab is a single-player camera game. A round lasts 60 active seconds. Kick a virtual pad with either foot, then lower the foot before kicking again. Consecutive hits increase the score multiplier. Easy uses larger pads and longer target windows; Normal uses smaller pads at varying heights below the calibrated hips.

## Playing locally

Run `python3 -m http.server 8765 --bind 127.0.0.1` from the repository, then open `http://localhost:8765/#taekwondo`. The static preview supports guest gameplay. Account and payment APIs require the existing Vercel development environment.
