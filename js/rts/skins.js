// ── SKINS SHOWCASE ──
// Static preview cards for the SKINS tab. Reuse the real in-game Arkship
// renderer (drawArkshipUnit, aerial2.js) so each skin matches what actually
// appears on the battlefield — held in 'attacking' mode with its twin beam
// emitters lit and firing, or 'phasing' mode with the shard-swirl dimmed and
// rippling as it prepares to release Prism and her Witches.
let skinsRAF=null;
let skinsFrame=0;
...
function drawArkshipSkinCard(canvasEl,mode){
  ...
  const fakeShip={frame:skinsFrame, arkMode:mode};
...
function skinsPreviewLoop(){
  ...
  if(atk) drawArkshipSkinCard(atk,'attacking');
  const pha=document.getElementById('skin-arkship-phasing-canvas');
  if(pha) drawArkshipSkinCard(pha,'phasing');
  skinsRAF=requestAnimationFrame(skinsPreviewLoop);
}

registerGame('skins', {
  init(){ if(!skinsRAF) skinsPreviewLoop(); },
