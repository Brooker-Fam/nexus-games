...
function executeCommand(cmd){
  ...
  const getBuildTypeFromEntity = (ent)=>{
    ...
    if(ent.isDarknessGen) return 'darknessgen';
    if(ent.isLightGen) return 'lightgen';
    return 'structure';
  };
  ...
  switch(cmd.type){
    ...
    case 'train_unit': {
      ...
      const darknessCost = cmd.unitType==='elite2' ? (cfg.elite2DarknessCost||0) : 0;
      const lightCost    = cmd.unitType==='elite'  ? (cfg.eliteLightCost||0)     : 0;
      if(darknessCost > 0 && (S.darkness[side]||0) < darknessCost) break;
      if(lightCost    > 0 && (S.light[side]||0)    < lightCost)    break;
      ...
      if(darknessCost>0) S.darkness[side]=Math.max(0,(S.darkness[side]||0)-darknessCost);
      if(lightCost>0)    S.light[side]   =Math.max(0,(S.light[side]||0)-lightCost);
      updateRtsHUD();
    ...
    case 'build_structure': {
      ...
      if(!worker) break;
      worker.buildTarget = { x:cmd.x, y:cmd.y, buildType:cmd.buildType||'structure', ghost:null };
    ...
    case 'assign_worker_task': {
      ...
      for(const workerId of cmd.workerIds||[]){
        ...
        if(cmd.task==='build'){
          ...
          worker.state = 'moving';
          worker.mineTimer = 0;
        } else if(cmd.task==='gather_darkness'){
          if(!target.isDarknessGen || target.underConstruction) continue;
          worker.buildTarget = null;
          worker.target = target;
          worker.assignedTask = 'gather_darkness';
          worker.assignedTargetId = target.id;
          worker.state = 'moving';
          worker.mineTimer = 0;
        } else if(cmd.task==='gather_light'){
          if(!target.isLightGen || target.underConstruction) continue;
          worker.buildTarget = null;
          worker.target = target;
          worker.assignedTask = 'gather_light';
          worker.assignedTargetId = target.id;
          worker.state = 'moving';
          worker.mineTimer = 0;
        }
