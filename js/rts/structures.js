...
function drawRTSStructure(rc, s){
  ...
  } else if(s.structType==='darknessgen'){
    // ── DARKNESS GENERATOR (Shadow) ── void crystal extractor
    const t=S.frame*0.05;
    rc.fillStyle='rgba(40,0,80,0.15)';
    rc.beginPath(); rc.ellipse(x,y+22,30,8,0,0,Math.PI*2); rc.fill();
    // extraction platform
    const platG=rc.createLinearGradient(x-22,y+14,x+22,y+22);
    platG.addColorStop(0,'#0a0015'); platG.addColorStop(1,'#050008');
    rc.fillStyle=platG; rc.beginPath(); rc.roundRect(x-22,y+12,44,10,2); rc.fill();
    rc.strokeStyle='rgba(100,0,180,0.5)'; rc.lineWidth=1; rc.stroke();
    // 3 jagged void shards rising from platform
    for(let i=0;i<3;i++){
      const sx2=x-14+i*14, sh=22+i%2*8;
      const sGrad=rc.createLinearGradient(sx2-5,y+12-sh,sx2+5,y+12);
      sGrad.addColorStop(0,'#220040'); sGrad.addColorStop(0.5,'#110020'); sGrad.addColorStop(1,'#050008');
      rc.fillStyle=sGrad;
      rc.beginPath(); rc.moveTo(sx2,y+12-sh); rc.lineTo(sx2-5,y+12); rc.lineTo(sx2+5,y+12); rc.closePath();
      rc.fill(); rc.strokeStyle='rgba(130,0,230,0.6)'; rc.lineWidth=0.8; rc.stroke();
      rc.strokeStyle=`rgba(160,60,255,${0.3+Math.sin(t+i)*0.2})`; rc.lineWidth=0.6;
      rc.beginPath(); rc.moveTo(sx2,y+12-sh+4); rc.lineTo(sx2,y+8); rc.stroke();
    }
    // swirling dark energy collector ring
    rc.strokeStyle=`rgba(100,0,200,${0.4+Math.sin(t*1.3)*0.2})`; rc.lineWidth=1.5;
    rc.beginPath(); rc.ellipse(x,y-2,18,9,t*0.5,0,Math.PI*2); rc.stroke();
    // central void orb
    const vOrb=rc.createRadialGradient(x,y-2,0,x,y-2,9);
    vOrb.addColorStop(0,'#cc66ff'); vOrb.addColorStop(0.5,'#660099'); vOrb.addColorStop(1,'transparent');
    rc.fillStyle=vOrb; rc.shadowColor='#aa00ff'; rc.shadowBlur=16;
    rc.beginPath(); rc.arc(x,y-2,9,0,Math.PI*2); rc.fill();
    // darkness level bar
    const darkPct=(s.darkness||0)/(s.maxDarkness||200);
    rc.shadowBlur=0; rc.fillStyle='rgba(40,0,60,0.7)'; rc.beginPath(); rc.roundRect(x+26,y-10,8,24,2); rc.fill();
    rc.strokeStyle='rgba(100,0,180,0.5)'; rc.lineWidth=0.8; rc.stroke();
    const lvH=Math.round(22*darkPct);
    rc.fillStyle=`rgba(140,0,220,${0.5+darkPct*0.5})`;
    rc.beginPath(); rc.roundRect(x+27,y+12-lvH,6,lvH,1); rc.fill();
    rc.font='6px Orbitron,sans-serif'; rc.textAlign='center'; rc.fillStyle=cfg.color;
    rc.fillText('DARK',x,y+34); rc.fillText('GEN',x,y+42);

  } else if(s.structType==='lightgen'){
    // ── LIGHT GENERATOR (Prism) ── crystal light spire
    const t=S.frame*0.04;
    rc.fillStyle='rgba(200,240,255,0.08)';
    rc.beginPath(); rc.ellipse(x,y+22,30,8,0,0,Math.PI*2); rc.fill();
    rc.strokeStyle='rgba(180,240,255,0.4)'; rc.lineWidth=1.5;
    rc.beginPath(); rc.ellipse(x,y+18,26,7,0,0,Math.PI*2); rc.stroke();
    // base platform
    const bpG=rc.createLinearGradient(x-22,y+12,x+22,y+22);
    bpG.addColorStop(0,'#c8f0ff'); bpG.addColorStop(1,'#88c8e8');
    rc.fillStyle=bpG; rc.beginPath(); rc.roundRect(x-22,y+12,44,8,2); rc.fill();
    rc.strokeStyle='rgba(100,200,255,0.5)'; rc.lineWidth=0.8; rc.stroke();
    // main crystal spire
    const spireG=rc.createLinearGradient(x-8,y-38,x+8,y+12);
    spireG.addColorStop(0,'#ffffff'); spireG.addColorStop(0.3,'#c8f0ff'); spireG.addColorStop(0.7,'#88ccee'); spireG.addColorStop(1,'#4499bb');
    rc.fillStyle=spireG;
    rc.beginPath(); rc.moveTo(x,y-42); rc.lineTo(x-8,y-12); rc.lineTo(x-10,y+12); rc.lineTo(x+10,y+12); rc.lineTo(x+8,y-12); rc.closePath();
    rc.fill(); rc.strokeStyle='rgba(180,240,255,0.6)'; rc.lineWidth=0.9; rc.stroke();
    // flanking crystals
    for(const [cx2,cy2,h2] of [[-16,y+4,18],[16,y+4,18]]){
      const smG=rc.createLinearGradient(x+cx2-4,cy2-h2,x+cx2+4,cy2);
      smG.addColorStop(0,'#eeffff'); smG.addColorStop(1,'#aaddee');
      rc.fillStyle=smG;
      rc.beginPath(); rc.moveTo(x+cx2,cy2-h2); rc.lineTo(x+cx2-4,cy2); rc.lineTo(x+cx2+4,cy2); rc.closePath();
      rc.fill(); rc.strokeStyle='rgba(160,230,255,0.5)'; rc.lineWidth=0.6; rc.stroke();
    }
    // pulsing light beam
    const beamAlpha=0.15+Math.sin(t*2)*0.1;
    rc.strokeStyle=`rgba(255,255,255,${beamAlpha})`; rc.lineWidth=3+Math.sin(t*2)*1;
    rc.beginPath(); rc.moveTo(x,y-42); rc.lineTo(x,y-80); rc.stroke();
    // orbiting light motes
    for(let i=0;i<3;i++){
      const oa=t+i*(Math.PI*2/3);
      rc.fillStyle=`rgba(220,250,255,${0.7+Math.sin(oa)*0.3})`; rc.shadowColor='#aaffff'; rc.shadowBlur=10;
      rc.beginPath(); rc.arc(x+Math.cos(oa)*14,y-10+Math.sin(oa)*6,2.5,0,Math.PI*2); rc.fill();
    }
    // light core glow
    const lcG=rc.createRadialGradient(x,y-18,0,x,y-18,12);
    lcG.addColorStop(0,'rgba(255,255,255,0.9)'); lcG.addColorStop(0.5,'rgba(180,240,255,0.5)'); lcG.addColorStop(1,'transparent');
    rc.fillStyle=lcG; rc.shadowColor='#aaffff'; rc.shadowBlur=18;
    rc.beginPath(); rc.arc(x,y-18,12,0,Math.PI*2); rc.fill();
    // light level bar
    const lightPct=(s.light||0)/(s.maxLight||200);
    rc.shadowBlur=0;
    rc.fillStyle='rgba(200,240,255,0.15)'; rc.beginPath(); rc.roundRect(x+26,y-10,8,24,2); rc.fill();
    rc.strokeStyle='rgba(100,200,255,0.4)'; rc.lineWidth=0.8; rc.stroke();
    const llH=Math.round(22*lightPct);
    rc.fillStyle=`rgba(200,240,255,${0.5+lightPct*0.5})`;
    rc.beginPath(); rc.roundRect(x+27,y+12-llH,6,llH,1); rc.fill();
    rc.font='6px Orbitron,sans-serif'; rc.textAlign='center'; rc.fillStyle=cfg.color;
    rc.fillText('LIGHT',x,y+34); rc.fillText('GEN',x,y+42);
