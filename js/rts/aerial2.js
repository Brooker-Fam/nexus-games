...
function drawArkshipUnit(rc,cfg,w){
  ...
  // back to whichever skin the player has equipped there (persisted, global —
  ...
  rc.shadowColor=cfg.color; rc.shadowBlur=isPhasing?14:26;
  rc.globalAlpha=isPhasing?0.6:1;

  if(isPhasing) drawArkshipPhasingCone(rc,cfg,w,isRoyal);
  else drawArkshipAttackingHull(rc,cfg,w,isRoyal);
...
// Phasing mode collapses the long hull into a short, squat cone (apex at
// the nose, a wide flared, ringed base at the tail) — a compact profile for
// slipping Prism and her Witches through, distinct from the attacking hull
// rather than just a dimmed version of it.
function drawArkshipPhasingCone(rc,cfg,w,isRoyal){
  ...
  rc.beginPath();
  rc.moveTo(apexX,0);
  rc.lineTo(baseX,-baseHalfH);
  rc.lineTo(baseX,baseHalfH);
  rc.closePath();
  const hg=rc.createLinearGradient(baseX,0,apexX,0);
  if(isRoyal){
    hg.addColorStop(0,'#7c8894'); hg.addColorStop(0.5,'#c9d2da'); hg.addColorStop(1,'#f4f7fa');
  } else {
